...
root = Tkinter.Tk()
root.withdraw()
file = tkFileDialog.askopenfilename(title='NTUA Tier Throttle')
try:
    wb = px.load_workbook(file, use_iterators = True)
    # set ws to active workbook, active sheet
    ws = wb.get_sheet_by_name(name = 'NTUA_Tier_Throttle_20150510')
except:
...
lineNo = 1
for row in ws.iter_rows():    # Read the throttle spreadsheet
    lineNo += 1
    if lineNo > 2:      # skip header
        if row[6].value:
            if row[6].value.strip() == "LTE_Profile":
                IMSI = str(row[3].value)
                IMSI = IMSI.strip()
                if len(IMSI) == 15:
                    rawratePlan = str(row[4].value)
                    rawratePlan = rawratePlan.strip()
                    temp = str(row[8].value)
                    rawstartDay = temp[9:12].strip()
                    #print "IMSI: " + IMSI + " rawratePlan: " + rawratePlan + " rawstartDay: " + rawstartDay + "," + row[8].value
