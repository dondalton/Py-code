...
os.chdir(home)
os.chdir("./Documents")
#os.chdir('C:\Users\DDalton\Documents\RF performance\CDMA\ODD\CDMA BSC-1')
...
for row in ws.iter_rows():
    ...
    if row[1].value < 1:    # end of cell parameters start of sys_para section
        break
    if row[52].value < 1 or row[53].value < 1:
        ...
        continue
    elif float(row[52].value) > 50.0 or float(row[52].value) < 20.0 or float(row[53].value) > -60.0 or float(row[53].value) < -125:
        whand.write(str(row[1].value) + " has a bad lat/lon value: " + str(row[52].value) + " / " + str(row[53].value) + '\n')
    cellLat[str(row[1].value)] = row[52].value  # populate dictionary with cell lat lon
    cellLon[str(row[1].value)] = row[53].value
    temp = str(row[1].value) + '-' + str(row[2].value)      # cellID-sector
