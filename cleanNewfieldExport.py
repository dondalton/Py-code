...
os.chdir(home)
os.chdir("./Documents")
os.chdir("C:\Users\DDalton\Documents\planning\mobility fund\ATNI_RAW_DATA_08062015\RAW_DATA_MEREGED\PHONE_DATA")
...
for files in os.listdir(path):
    ...
    if os.path.splitext(files)[1] == ".txt":
        ...
        for line in fhand:
            fields = line.split("\t")
            if len(fields) > 8:
                if len(fields[1]) > 1 and len(fields[2]) > 1 and len(fields[7]) > 1:
                    #print fields[1].translate(None,'"') + "," + fields[2].translate(None,'"') + "," + fields[3].translate(None,'"' + "\n")
                    whand.write(fields[1].translate(None,'"') + "," + fields[2].translate(None,'"') + "," + fields[7].translate(None,'"') + "," + fields[8].translate(None,'"') + "\n")
